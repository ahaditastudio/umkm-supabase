...
const variants: Record<ButtonVariant, string> = {
  primary: "bg-primary text-white hover:bg-primary/90 shadow-[0_1px_2px_rgba(0,0,0,0.05)] border border-primary/10",
...
export function Button({ className, variant = "primary", size = "md", loading, children, disabled, ...props }: ButtonProps) {
  return (
    <button
      className={cn(
        "inline-flex items-center justify-center gap-2 rounded-xl font-medium transition duration-200 select-none disabled:pointer-events-none disabled:opacity-50 btn-press",
        variants[variant],
        sizes[size],
        className,
      )}
